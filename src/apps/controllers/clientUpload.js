...
module.exports.postImage = function (req, res, next) {
  ...
  form.parse(req, function (err, fields, files) {
		...
		var success = true;
		fs.mkdir(`./src/public/uploads/${folder}`, async function(err){
			...
			await fileKey.forEach(async function(key){
				...
				var newpath = `./src/public/uploads/${folder}/` + fields.student_name + "_" + key + ".jpg"
			...
			try{
				await scp({
					...
					client.uploadDir(`./src/public/uploads/${folder}`, `${server_upload_dir}/${folder}`)
						.then(response => {
